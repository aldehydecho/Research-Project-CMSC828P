...
input_size = os.path.getsize(args.input_model)
with open(args.input_model, 'rb') as f_in:
    for _ in range(0, input_size, max_bytes):
        bytes_in += f_in.read(max_bytes)

flda_model = pickle.loads(bytes_in)

flda_model.mutations2docs_test_and_initialize(test_mutation)

for i in range(0, 100+1):
    flda_model.testUpdate(i)

bytes_out = pickle.dumps(flda_model)
file_path = args.output_model
max_bytes = 2**31 - 1
