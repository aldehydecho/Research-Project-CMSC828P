...
class flda():
    ...
    def doSampling(self, iteration):
        ...
        if iteration == 0:
            ...
            for d in range(0, self.doc_num):
                ...
                for w in range(0, self.mutation_num):
                    ...
                    # self.nZW[unique_factor, w] += counts
                    # self.nZ[unique_factor] += counts
                    # self.nDZ[d, unique_factor] += counts
            ...
            print('\tsampling_time ', end-start)

        start = time.time()
        self.updateWeights(iteration)
        end = time.time()
        print('\tupdating_weights_time ',end-start)
        ...
        print('\tupdating_prior_time ',end-start)
        ...
        if iteration % 5  == 0:
            ...
            print('\tLL_time ',end-start)
                
    
                    
    def sample(self, d, n):
        w = self.docs[d][n]
        factor_index = self.docsZ[d][n]
        
        self.nZW[factor_index, w] -= 1
        self.nZ[factor_index] -= 1
        self.nDZ[d][factor_index] -=1
        
        prob = self.priorZW[:, w]/np.sum(self.priorZW[:, w])
        new_factor_index = np.random.choice(self.factor_total, p = prob)
        
        self.docsZ[d][n] = new_factor_index
                
        self.nZW[new_factor_index,w] += 1
        self.nZ[new_factor_index] += 1
        self.nDZ[d,new_factor_index] += 1

flda_model = flda([26,2])
flda_model.mutations2docs(sbs96_df.values, sbs_names)
...
for i in range(0, 400+1):
    ...
    if i % 20 == 0:
        ...
        file_path = "output/storage_"+str(i)+".pkl"
        max_bytes = 2**31 - 1
